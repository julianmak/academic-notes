...
\begin{example}
  ...
  \end{equation*}
  and the eigenvalues satisfy\sidenote{Could have done this with the trace and
...
the reals\sidenote{This one is actually not diagonalisable over $\mathbb{C}$
either, since it is \textbf{nilpotent} (of index 2; $\As^2 = 0$ here).}.
...
\begin{example}
  \begin{enumerate}
    ...
    Hamiltonian of the system\sidenote{And has additional properties that make
    the eigenvalues and eigenvectors have certain properties; more later when we
    talk about \emph{Hermitian}/\emph{self-adjoint} matrices.}, and
    ...
    compute.\sidenote{In the context of a Markov chain, the transition matrix
...
the following:
\begin{theorem}
  A matrix $\As$ is orthogonal if and only if $\As^{-1} = \As^T$.\sidenote{So
...
\begin{lemma}\label{thm:real_sym_eigen}
  Let $\As$ be a real symmetric matrix. Then the eigenvalues are all real, and
  the eigenvectors are mutual orthogonal to each other.
\end{lemma}
...
\As^{-1}}$.\sidenote{These are used a lot in quantum mechanics.} Again, if $\As$
is actually real, then this reduces to the definition of an orthogonal matrix.

\begin{theorem}
  Let $\As$ be a Hermitian matrix. Then the eigenvalues are all real, and the
  eigenvectors are mutual orthogonal to each other.\sidenote{Compare this with
