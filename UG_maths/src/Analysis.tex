...
that spans the whole of $M$ is called an \textbf{atlas}.\marginnote{Notice then
in the previous proof, $\phi$ is a chart, and $W \cap \{(\boldsymbol{x},
\boldsymbol{y}) \in \mathbb{R}^n\ |\ f(\boldsymbol{x}, \boldsymbol{y}) =
\boldsymbol{c}\}$ is a $k$-manifold.}
...
\begin{example}
  ...
  1\}$.\marginnote{Note that $2$-sphere would be the standard sphere, which is
  two-dimensional with zero volume.} With $f : \mathbb{R}^{n+1} \to \mathbb{R}$
