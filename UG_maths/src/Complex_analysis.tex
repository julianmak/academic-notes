...
group}.\marginnote{Which, given the suggested link with $2\times2$ matrices with
non-vanishing determinant, is maybe expected.}
...
One such case is the case of a uniform flow past a disk. For simplicity, we
consider the unit disk, and the domain is thus $\mathbb{C} \setminus D(1, 0)$.
For the harmonic problems, it is easier to work in polar co-ordinates, and the
Laplace equation for the potential $\phi$ is given by
\begin{equation*}
  \frac{1}{r}\frac{\partial}{\partial r} \left(r\frac{\partial\phi}{\partial r}\right) + \frac{1}{r^2} \frac{\partial^2 \phi}{\partial \theta^2} = 0,
\end{equation*}
which in this case is subject to boundary conditions at infinity and Neumann
boundary conditions at the boundary of the unit disk
\begin{equation*}
  \phi \sim Ur\cos\theta \quad \textnormal{as} \quad r \to \infty, \qquad
  \frac{\partial\phi}{\partial r} = 0 \quad \textnormal{on} \quad r=1.
\end{equation*}
By separation of variables, one can show that
\begin{equation*}
  \phi = U\left(r + \frac{1}{r}\right) \cos\theta,
\end{equation*}
where $U$ is the free-stream uniform flow, directed in the $x$ direction. A
similar argument leads to
\begin{equation*}
  \psi = U\left(r - \frac{1}{r}\right) \sin\theta.
\end{equation*}
The complex potential is then
\begin{align*}
  F &= U\left[\left(r + \frac{1}{r}\right) \cos\theta + \zi \left(r - \frac{1}{r}\right) \sin\theta\right] \\
    &= U\left(z + \frac{1}{z}\right),
\end{align*}
since $z = r\ex^{\zi\theta}$.\marginnote{Note this potential leads to no lift,
since there is no circulation $\Gamma = \oint_\gamma
\boldsymbol{u}\cdot\mathrm{d}\boldsymbol{l}$. The application of the
\textbf{Kutta condition} adds a point vortex term $\zi\Gamma/(2\pi)\log z$ so
that the closed contour integral to compute the circulation is non-zero.}

A particular choice of practical relevance is the \textbf{Joukowsky map}
\begin{equation*}
  w = f(z) = z + \frac{1}{z}.
\end{equation*}
The function is holomorphic except at infinity, and noting that $f'(z) = 1 - 1 /
z^2 \neq 0$ except at $z=\pm 1$ (the leading and trailing edge of the unit disk)
and $z=0$ (which is not actually in the domain anyway), thus is it conformal
away from those special points. In this instance, the unit circle is mapped onto
the line segment going from $w=-2$ to $w=2$, and the potential is conformally
mapped onto the domain away from the line segment, and thus the transformed
potential and streamfunction are still valid potential and streamfunction for
the irrotational and incompressible flow problem. New solutions can be generated
accordingly (e.g. a unit disk not centered at the origin), which has
applications in aerofoil theory in aeronautics.
